...
* Last Modified time: 2025-12-10 14:10:24
*----------------------------------------------------------------------------*/
#include <cassert>
...
void CouplingConstant::create(const int& num_types) 
{
  assert(num_types>=0);
  super_type::clear();
...
void CouplingConstant::add_type(const int& type, const std::string& expr)
{
  assert(type>=0);
...
void CouplingConstant::add_type(const int& type, 
...
//-----------------------ModelTerm-------------------------
void ModelTerm::construct(const std::string& name, const op::quantum_op& op, 
    const CouplingConstant& cc, const unsigned& size)
{
  if (!cc.valid()) throw std::invalid_argument("ModelTerm:: Invalid CouplingConstant");
  name_ = name;
  op_ = op;
  cc_ = cc;
...
void ModelTerm::eval_coupling_constant(const ModelParams& cvals, const ModelParams& pvals)
{
