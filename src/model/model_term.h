...
public:
  ...
  void create(const int& num_types);
  void add_type(const int& type, const std::string& expr);
  void add_type(const int& type, const std::vector<std::string>& expr_vec);
  void add_type(const int& type, const strMatrix& expr_mat);
...
public:
  ...
  ModelTerm() {}
  ModelTerm(const std::string& name, const op::quantum_op& op, const CouplingConstant& cc, 
