...
* All rights reserved.
* Date:   2025-12-06 08:33:27
* Last Modified by:   Amal Medhi
* Last Modified time: 2025-12-10 08:35:24
*----------------------------------------------------------------------------*/
#include "model.h"

namespace model {

int Model::add_siteterm(const std::string& name, const CouplingConstant& cc, 
  const op::quantum_op& op)
{
  ...
  int num_sitetypes = sitetypes_map_.size();
  this->site_terms_.push_back(ModelTerm(name,op,cc_remapped,num_sitetypes));
  return this->site_terms_.size();
}

int Model::add_bondterm(const std::string& name, const CouplingConstant& cc,
  const op::quantum_op& op)
{
  ...
  int num_bondtypes = bondtypes_map_.size();
  bond_terms_.push_back(ModelTerm(name, op, cc_remapped, num_bondtypes));
...
/*void Model::change_parameter_value(const std::string& pname, const double& pval) 
{
  auto it = parms_.find(pname);
  if (it != parms_.end()) it->second = pval;
}*/

void Model::update_parameter(const std::string& pname, const double& val)
{
  parms_.at(pname) = val; 
}
...
double Model::get_parameter_value(const std::string& pname) const
{
  auto it = parms_.find(pname);
  if (it != parms_.end()) return it->second;
  else throw std::logic_error("Model::get_parameter_value: parameter does not exist");
}
...
{
  std::ostringstream info_strm, signature_strm;
  info_strm.clear();
